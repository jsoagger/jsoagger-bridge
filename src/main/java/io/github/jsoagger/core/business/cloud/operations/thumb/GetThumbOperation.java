 ...
 *
 */
public class GetThumbOperation implements IOperation {
  ...
  public void doOperation(JsonObject query, Consumer<IOperationResult> resultHandler, Consumer<Throwable> exHandler) {
    try {
      ...
      if (cache.contains(cacheKey, CachedObjectType.IMAGE)) {
        ...
        result.addMetaData("thumb", res);
        resultHandler.accept(result);
      }
      else {
        final byte[] res = CloudServicesLocator.thumbedApi.getThumb(query);
        if (res == null) {
        ...
        result.addMetaData("thumb", clone);
        resultHandler.accept(result);
      }
    }
